...
const cardsData = [
    {
        key: 'trampoline',
        variants: [
            { img: trampoline, label: 'Trampoline' },
            { img: carrom, label: 'Jump Bounce' },
            { img: trampoline, label: 'Spring Fun' },
        ],
    },
    {
        key: 'carrom',
        variants: [
            { img: carrom, label: 'Carrom' },
            { img: trampoline, label: 'Board Game' },
            { img: carrom, label: 'Strike & Pocket' },
        ],
    },
    {
        key: 'jumping',
        variants: [
            { img: jumping, label: 'Jumping' },
            { img: karting, label: 'Bounce High' },
            { img: jumping, label: 'Leap Fun' },
        ],
    },
    {
        key: 'karting',
        variants: [
            { img: karting, label: 'Go Karting' },
            { img: carrom, label: 'Speed Track' },
            { img: karting, label: 'Drift Ride' },
        ],
    },
    {
        key: 'football',
        variants: [
            { img: football, label: 'Table Football' },
            { img: jumping, label: 'Foosball' },
            { img: football, label: 'Kickoff Match' },
        ],
    },
];

export default function Rides() {
    const [currentIndices, setCurrentIndices] = useState(Array(cardsData.length).fill(0));
    const [flipping, setFlipping] = useState(Array(cardsData.length).fill(false));
        ...
        <div className={`relative ${customClass}`}>
            <div className={`flip-card w-full h-full ${flipping[cardIndex] ? 'flip' : ''}`}>
                ...
                <span className="absolute bottom-0 right-0 bg-[#009D48] text-white text-xl px-3 py-1">
                    {cardsData[cardIndex].variants[currentIndices[cardIndex]].label}
                </span>
            </div>
        </div>
    );
